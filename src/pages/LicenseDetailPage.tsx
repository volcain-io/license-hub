...
import { ArrowLeft, Copy, Power, PowerOff, Trash2, User, Mail, Package, Key, Clock, Hash, FileText } from 'lucide-react';
  ...
  return (
    <>
      ...
      <PageHeader
        ...
        description={`License detail and activation history`}
        actions={<StatusBadge status={license.status} />}
      />
          ...
          <Table>
            ...
            <TableBody>
              {licenseActivations.map(a => (
                <TableRow key={a.id}>
                  <TableCell className="font-medium text-sm">{a.deviceName}</TableCell>
                  <TableCell className="font-mono text-xs text-muted-foreground">{a.deviceFingerprint}</TableCell>
